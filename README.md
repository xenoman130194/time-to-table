# Time-To-Table v0.6.7 RC (Калькулятор для ленивых)
...
Цель создания - ускорение ввода данных в SAP, после выгрузки .xml данные из него копируются -> в SAP ручками через ctrl-c/v.
...
- Экспорт в XML (.xml) - выгружает всю историю расчётов в файл, формулы работают (можно менять время и пересчитается)
- Экспорт JSON - сохраняет техкарты в файл
- Импорт JSON - загружает техкарты из файла

## История

- Можно скачать историю в XML
- Можно очистить историю

## Z7

- Генерирует текст для Z7 отчёта
- В XML тоже выгружается
...
- [x] Добавить настройки (тема, дефолтное время обеда итд)
- [ ] Разбить main.js на модули
- [ ] Добавить TypeScript, а надо ли?
- [x] Добавить горячие клавиши(частично сделано)
...
- Релиз-кандидат перед финальным релизом 0.6.7: небольшие исправления и улучшения.
- Исправлены мелкие баги UX и поведение модальных окон; обновлён заголовок окна приложения.
- Улучшена стабильность экспорта и инициализации данных при старте.
