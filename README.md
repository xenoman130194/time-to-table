...
> Примечание: в репозитории удалены сгенерированные артефакты сборки (папка `src-tauri/target/`).
> Чтобы собрать заново, выполните инструкции в разделе "Сборка" ниже.
...
### История
- Все расчёты сохраняются в сессии
...
**localStorage** 
   - все данные хранятся в localStorage WebView2 
   - при очистке кэша браузера всё потеряется
   - нет синхронизации между компами, решается импортом/экспортом JSON
   - удобнее было бы ввести БД или отдельный файл
...
Если надо сбросить всё - удалить эту папку.
...
-### v0.6.2
-
-- Maintenance: removed tracked build artifacts (`src-tauri/target/`) from repository.
-
-### v0.6.1
- Перенос на Tauri v2
